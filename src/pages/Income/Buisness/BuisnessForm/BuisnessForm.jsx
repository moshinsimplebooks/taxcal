...
import EmpBuisYup from '../../../../Utils/validation/EmpBuisYup/EmpBuisYup';
import { NumericFormat } from 'react-number-format';
import ResponseHandler from '../../../../Utils/ResponseHandler/ResponseHandler';
import { v4 as uuidv4 } from 'uuid';
import { addTax } from '../../../../Features/Taxer/taxSlice'

export default function BuisnessForm() {
    const dispatch = useDispatch()
    const taxes = useSelector((state) => state.taxes)

    const filterBuisnessTaxes = taxes.filter(tax => tax.source === "empBuisness")
    ...
    const { values, handleChange, handleSubmit, errors, touched } = useFormik({
        ...
        validationSchema: EmpBuisYup.createBuisnessIncome,
        onSubmit: (values) => {
            try {
            ...
            <form className='needs-validation' noValidate onSubmit={handleSubmit}>
                ...
                <div className="row mb-1">
                    <div className="col-12">
                        <button type='submit' className='btn btn-success w-100'>Add Buisness Income</button>
                    </div>
                </div>
            </form>
