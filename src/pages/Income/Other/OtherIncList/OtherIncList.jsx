...
export default function OtherIncList() {
    const taxes = useSelector((state) => state.taxes)
    const filterOtherTaxes = taxes.filter(tax => tax.source === "empOtherInc")

    const dispatch = useDispatch()
    const handleDeletes = (id) => {
        // Filter out the todo item with the specific id
        dispatch(removeTax(id))
        const newTaxes = filterOtherTaxes.filter((tax) => tax.id !== id);
        ...
        <div className="col-sm-7 mb-3 mb-sm-0">
            <div className="card border-0">
                <div className="card-body py-0">
                    <h5 className="card-title">Other Income</h5>
                    ...
                    <div>
                        <table className="table table-striped" >
                            ...
                            <tbody>
                                {
                                    filterOtherTaxes.length > 0 ? (
                                        filterOtherTaxes.map((tax) => (
                                            <tr key={tax.id}>
                                                <td>{tax.type}</td>
